...
class MyServerProtocol(WebSocketServerProtocol):
    ...
    def __init__(self, process_pose=False, process_faces=False, saveLocation='D:/AimeeImageResults/PulledImages/'):
        ...
        #define all open pose variables that need initializing as class variables
        self.setupOP()

    def setupOP(self):
        self.logger = logging.getLogger('TfPoseEstimator-Video')
        ...
        #logger.debug('initialization %s : %s' % (get_graph_path(model)))
        self.w = 432
        self.h = 368
        self.e = TfPoseEstimator(get_graph_path(self.model), target_size=(self.w, self.h))

        csvfile = open('D:/AimeeImageResults/bodyparts.csv', 'a', newline='')
        myFields = ['frameNumber', 'personNumber', 'partNumber', 'xCoordinate', 'yCoordinate', 'score']
        self.partwriter = csv.DictWriter(csvfile, delimiter=' ', quotechar='|', quoting=csv.QUOTE_MINIMAL, fieldnames=myFields)
        self.partwriter.writeheader()
...
##            partwriter.writeheader()
    ...
    def recognizeAndReset(self):
        if self.imageCount >= self.imageWait:
            self.imageCount = 0
            prepImages(self)
            faceRecognize(self.images,self)#Does not work, pay no attention
...
def openpose(images,self):
    ...
    for imageName in images:
        ...
        elapsed = time.time() - t
        self.logger.info('read and inferenced image: %s in %.4f seconds.' % ("D:/AimeeImageResults/Images/"+ imageName, elapsed))
        imCount += 1
        
        t = time.time()
        for personNum in range(len(humans)):
            for partKey, part in humans[personNum].body_parts.items():
               self.partwriter.writerow({'frameNumber':imageName,
                                    ...
                                    'score':part.score})
        elapsed = time.time()-t
        self.logger.info('wrote parts from image: %s in %.4f seconds.' % ("D:/AimeeImageResults/Images/"+ imageName, elapsed))
